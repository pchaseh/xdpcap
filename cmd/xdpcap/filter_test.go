...
func TestMissingFilter(t *testing.T) {
	_, err := newFilterWithMap(hookMap(t, 1), nil, testOpts())
	if err == nil {
...
func TestFilterProgramForAllModes(t *testing.T) {
	testFunc := func(xdpFragsEnabled bool) func(t *testing.T) {
		return func(t *testing.T) {
			...
			filter, err := newFilterWithMap(hookMap, nil, opts)
			if err != nil {
				t.Fatal(err)
			}
			filter.close()
...
func TestAllActions(t *testing.T) {
	...
	// progs with actions from 0-9. Only 0-3 are used currently.
	filter, err := newFilterWithMap(hookMap(t, 10), nil, opts)
	if err != nil {
		t.Fatal(err)
	}
	defer filter.close()
...
func mustNew(t *testing.T, opts filterOpts) *filter {
	...
	filter, err := newFilterWithMap(hookMap(t, len(opts.actions)), nil, opts)
	if err != nil {
		t.Fatal(err)
	}
